...
def deter_model(aggregated_demand_period_df: pd.DataFrame,
    ...
    model.addConstraint(xp.Sum(aggregated_demand_period_df.loc[(c, p), t] * x[w, c, t] for c in C for p in P) <=
                        candidate_df.loc[w, 'Capacity'] * o[w, t] for w in W for t in T)
    model.addConstraint(o[w, t] >= o[w, t - 1] for w in W for t in T if t != 0)
    model.addConstraint(xp.Sum(z[w, s, t] for s in S_P_dict[p]) ==
                        xp.Sum(aggregated_demand_period_df.loc[(c, p), t] * x[w, c, t] for c in C)
    ...
    Tra_w_c_cost = xp.Sum(cost_w_to_cluster.loc[w, c] * aggregated_demand_period_df.loc[(c, p), t] * x[w, c, t]
    ...
    def print_results():
        ...
        opened_warehouse = []
        for t in T:
            setup_warehouse_in_t = [w for w in W if y_sol[w] == 1 and o_sol[w, t] == 1 and w not in opened_warehouse]
