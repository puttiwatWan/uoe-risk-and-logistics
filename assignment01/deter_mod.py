...
def deter_model(agg_dem_cus_period_df:pd.DataFrame, 
    ...
    for w in W:
        ...
        for t in T:
            ...
            for c in C:
                model.addConstraint(xp.constraint(x[w,c,t] <= y[w,t]))  

    for t in T:
        for c in C:
            model.addConstraint(xp.constraint(xp.Sum(x[w,c,t] for w in W) == 1))
            # model.addConstraint(xp.constraint(xp.Sum(x[w,c,t] for w in W) >= 1))
            for p in P:
                Demand = agg_dem_cus_period_df.loc[(c,p),t]
                # model.addConstraint(xp.constraint(xp.Sum(v[w,c,p,t] for w in W) >= Demand))
                for w in W:
                    model.addConstraint(xp.constraint(v[w,c,p,t] == Demand * x[w,c,t]))    
                    # model.addConstraint(xp.constraint(v[w,c,p,t] <= Demand * x[w,c,t]))    
